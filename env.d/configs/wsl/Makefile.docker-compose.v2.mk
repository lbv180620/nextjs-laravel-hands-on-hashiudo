...
#^ WSLではinfra/dataにdb-storeをバインドしないので@make $(db)-setが不要に。
launch:
	@make chinfra
	@make file-set
...
chmk:
	cp env/configs/mac/Makefile Makefile

#^ WSLでトップレベルの名前付きボリュームをローカルのディレクトリにバインドする場合に使用。
#^ PowerShellで実行する。
#^ 実行後、WSL側で make chown-dataでパーミッションを解決する。
mount:
...
#^ WSLではinfra/dataにdb-storeをバインドしないのでこちらを採用。
# file-set:
# 	mkdir -p sqls/sql sqls/script infra/data infra/redis infra/pgadmin4
# 	touch sqls/sql/query.sql sqls/script/set-query.sh
# 	mkdir -p .vscode && cp env/docs/*.json .vscode
# 	cp env/docs/docker-compose.env .env
# 	mkdir -p backend
# # mkdir -p backend frontend
file-set:
	mkdir -p infra/redis infra/pgadmin4
	mkdir -p .vscode && cp env/docs/*.json .vscode
	cp env/docs/docker-compose.env .env
	mkdir -p backend
...
#^ WSLではinfra/dataにdb-storeをバインドしないのでこちらを採用。
# file-delete:
# 	rm -rf infra/data infra/redis infra/pgadmin4 sqls .vscode && rm .env
# 	rm -rf backend frontend infra/redisinsight infra/phpmyadmin
file-delete:
	rm -rf infra/redis infra/pgadmin4 .vscode && rm .env
	rm -rf backend frontend infra/redisinsight infra/phpmyadmin


# **** SQLファイルの設定 ****

#^ WSLではinfra/dataにdb-storeをバインドしないのでこちらが不要に。
...
tailwind-set:
	...
	cp -r env/tailwind/tailwind.config.js frontend/
	cp -r env/tailwind/postcss.config.js frontend/
...
build:
	docker compose build --no-cache --force-rm $(name)
...
# インフラを再ビルド
#^ WSLではinfra/dataにdb-storeをバインドしないので@make chown-$(db)が不要に。
init:
	@make rebuild
# @make chown-$(db)
	@make groupadd
...
#^ WSLではinfra/dataにdb-storeをバインドしないので使えない。
# sqlc:
# 	@make query
...
comp-add-D-pest:
	docker compose exec $(ctr) composer require pestphp/pest --dev --with-all-dependencies
	docker compose exec $(ctr) composer require pestphp/pest-plugin-laravel --dev
	docker compose exec $(ctr) php artisan pest:install
	@make pest-init
	@make pest

pest-init:
	docker compose exec $(ctr) ./vendor/bin/pest --init

pest:
	docker compose exec $(ctr) ./vendor/bin/pest
...
comp-add-seotools:
	docker compose exec $(ctr) composer require artesaos/seotools
...
comp-add-link-preview:
	docker compose exec $(ctr) composer require dusterio/link-preview
...
# https://www.techpit.jp/courses/176/curriculums/179/sections/1194/parts/4690
...
# ここではdusterio/link-previewパッケージのDusterio\LinkPreview\Clientというクラスを使っています。Clientクラスに URL を渡すとその URL が示すページを開き、<title></title>タグなどからメタ情報を引っ張ってくることができます。

# 引っ張ってきたデータをコメントやユーザー ID 等と紐付けてまとめてデータベースに保存します。
...
#^ v1.11.0: Laravel 8 don't support the vite.
# https://github.com/laravel/breeze/releases
#^ v1.7.0からLaravel 9をサポートし始めた。
# Laravel 8以降で使用する場合は、v1.6.*以降

install-breeze:
	docker compose exec $(ctr) php -d memory_limit=-1 /usr/bin/composer require --dev "laravel/breeze=$(v)"
